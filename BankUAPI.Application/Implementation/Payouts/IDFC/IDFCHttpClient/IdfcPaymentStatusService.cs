...
{
    public sealed class IdfcPaymentStatusService : IIdfcPaymentStatusService
    {
        ...
        public IdfcPaymentStatusService(
        ...
        public async Task<PaymentStatusEnquiryResponse> EnquireAsync(
            ...
            string clientCode)
        {
            ...
            {
                if (cached.StartsWith("Idempotency"))
                    throw new InvalidOperationException(cached);

                return JsonSerializer.Deserialize<PaymentStatusEnquiryResponse>(cached)!;
            }
            ...
            var result =
                JsonSerializer.Deserialize<PaymentStatusEnquiryResponse>(decryptedJson)!;

            var rd = result.paymentTransactionStatusResp.resourceData;

            await _ledger.RecordBalanceAsync(
                clientCode: clientCode,
                internalTxnId: rd.transactionReferenceNumber,
                debitAccount: "",
                status: result.paymentTransactionStatusResp.metaData.Status,
                requestJson: requestJson,
                responseJson: decryptedJson,
                TxnType: "PAYMENT_STATUS",
                BankCode: "IDFC",
                TransactionType: rd.transactionType,
                ExternalTxnId: rd.paymentReferenceNumber,
                BankResponseCode: rd.errorId,
                BankResponseMessage: rd.errorMessage
            );
